...
export default function ProductPrintAreaConfigurator() {
  ...
  const handleFileUpload = async (file, target = "current-view") => {
    if (!file) return;
    ...
    try {
      ...
      if (res.ok && data.success && data.url) {
        if (target === "current-view") {
          handleUpdateViewImage(data.url);
          showToast("Uploaded & stored in Shopify Files!");
        } else if (target === "new-front") {
          setNewProductForm((prev) => ({ ...prev, frontImage: data.url }));
          showToast("Front mockup uploaded to Shopify Files!");
        } else if (target === "new-back") {
          setNewProductForm((prev) => ({ ...prev, backImage: data.url }));
          showToast("Back mockup uploaded to Shopify Files!");
        }
      } else {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 flex flex-col gap-5">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-3xl p-5 border border-gray-200 dark:border-gray-700 shadow-sm space-y-4">
              ...
              {/* Active Image Preview & Action Buttons */}
              <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700/60">
                <div className="w-16 h-16 rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 flex items-center justify-center p-1 overflow-hidden shrink-0">
                  {currentView.image ? (
                    <img src={currentView.image} alt={currentView.label} className="max-h-full max-w-full object-contain" />
                  ) : (
                ...
                <div className="flex-1 min-w-0">
                  <span className="text-xs font-bold text-gray-800 dark:text-white block truncate">
                    {currentView.image ? currentView.image.split("/").pop() : "No image selected"}
                  </span>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <input
                      ...
                      onChange={(e) => {
                        if (e.target.files?.[0]) {
                          handleFileUpload(e.target.files[0], "current-view");
                        }
                      }}
                    />
          ...
          <div className="bg-white dark:bg-gray-800 rounded-3xl max-w-lg w-full shadow-2xl border border-gray-200 dark:border-gray-700 p-6 animate-in fade-in zoom-in-95">
            ...
            <form onSubmit={handleCreateProduct} className="space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 p-2.5 bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700/60">
                  <div className="w-12 h-12 rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 flex items-center justify-center p-1 overflow-hidden shrink-0">
                    {newProductForm.frontImage ? (
                      <img src={newProductForm.frontImage} alt="Front Mockup" className="max-h-full max-w-full object-contain" />
                    ) : (
                  ...
                  <div className="flex-1 min-w-0 flex items-center gap-2">
                    <input
                      type="file"
                      ref={addModalFrontInputRef}
                      accept="image/*"
                      className="hidden"
                      onChange={(e) => {
                        if (e.target.files?.[0]) {
                          handleFileUpload(e.target.files[0], "new-front");
                        }
                      }}
                    />
                    <button
                      type="button"
                      onClick={() => addModalFrontInputRef.current?.click()}
                      className="px-2.5 py-1.5 bg-brand-500 hover:bg-brand-600 text-white rounded-lg text-xs font-bold shadow-xs flex items-center gap-1 cursor-pointer"
                    >
                      <FiUploadCloud className="w-3.5 h-3.5" /> Upload
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className="px-2.5 py-1.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-xs font-bold text-gray-700 dark:text-gray-200 flex items-center gap-1 cursor-pointer"
                    >
                      <FiImage className="w-3.5 h-3.5 text-brand-500" /> Shopify Files
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-3 p-2.5 bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700/60">
                  <div className="w-12 h-12 rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 flex items-center justify-center p-1 overflow-hidden shrink-0">
                    {newProductForm.backImage ? (
                      <img src={newProductForm.backImage} alt="Back Mockup" className="max-h-full max-w-full object-contain" />
                    ) : (
                      <FiImage className="w-5 h-5 text-gray-400" />
                    )}
                  </div>
                  <div className="flex-1 min-w-0 flex items-center gap-2">
                    <input
                      type="file"
                      ref={addModalBackInputRef}
                      accept="image/*"
                      className="hidden"
                      onChange={(e) => {
                        if (e.target.files?.[0]) {
                          handleFileUpload(e.target.files[0], "new-back");
                        }
                      }}
                    />
                    <button
                      type="button"
                      onClick={() => addModalBackInputRef.current?.click()}
                      className="px-2.5 py-1.5 bg-brand-500 hover:bg-brand-600 text-white rounded-lg text-xs font-bold shadow-xs flex items-center gap-1 cursor-pointer"
                    >
                      <FiUploadCloud className="w-3.5 h-3.5" /> Upload
                    </button>
